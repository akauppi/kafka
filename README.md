...
*Note that if building the jars with a version other than 2.10, you need to set the `SCALA_VERSION` and `SCALA_BINARY_VERSION` variables or change the defaults in `bin/kafka-run-class.sh` to run the quick start.*
